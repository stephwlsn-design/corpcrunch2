...
export function generateStructuredData(postData) {
  ...
  const authorName =
    [authorFirstName, authorLastName].filter(Boolean).join(' ') || 'Corp Crunch';
  const published = publishDate || publishedAt || createdAt || new Date().toISOString();
  const readMinutes = readingTimeMinutes || readingTime || calculateReadingTime(content);
  ...
  const baseStructure = {
    ...
    image: ogImage || bannerImageUrl,
    datePublished: published,
    dateModified: updatedAt || published,
    author: {
      '@type': 'Person',
      name: authorName,
    },
    publisher: {
      ...
      logo: {
        '@type': 'ImageObject',
        url: 'https://www.corpcrunch.io/assets/img/logo/logo.png',
      },
    },
  ...
  return Object.fromEntries(
    Object.entries(baseStructure).filter(([, value]) => value !== undefined)
  );
}
